    ...
    def __init__(self, collate_fns, keys):
        ...
        for key, collate_fn in zip(keys, collate_fns):
            if isinstance(collate_fn, dict):
                collate_fn_copy = collate_fn.copy()
                func = collate_fn_copy.pop('type')
                self.collate_fns[key] = partial(func, **collate_fn_copy)
            else:
                self.collate_fns[key] = collate_fn

    def __call__(self, data_samples):
        data_samples = [data_sample for data_sample in data_samples if data_sample and len(data_sample) > 0]
        if not data_samples:
            return {'data': {}, 'data_samples': None}

        # Group data samples by their type ('text2image' vs 'image2image')
        grouped = {}
        ...
        for key, samples in grouped.items():
            if key in self.collate_fns:
                data_dict[key] = self.collate_fns[key](samples)['data']
