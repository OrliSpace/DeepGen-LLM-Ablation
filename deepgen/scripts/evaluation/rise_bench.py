...
class RISEBench(Dataset):
    def __init__(self, data_path, width=512, height=512):
        self.data_path = data_path
        self.width = width
        self.height = height
        with open ('RISEBench-full/datav2_total_w_subtask.json', "r") as f:
    ...
    def __getitem__(self, idx):
        ...
        data_dict.update(idx=idx, image_pixel_src=image_pixel_src, index = data_dict['index'],category=data_dict['category'])
        return data_dict
        
if __name__ == '__main__':
    parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('--config', default='configs/models/deepgen_sft_llm_adapter.py', type=str, help='Path to mmengine model config')
    parser.add_argument('--checkpoint', default=None, type=str)
    parser.add_argument('--batch_size', default=4, type=int)
    parser.add_argument('--data', default='evaluation/RISEBench-full', type=str)
    parser.add_argument('--output', default='result_rise', type=str)
    ...
    dataset = RISEBench(data_path=args.data, width=args.width, height=args.height)
    ...
    for batch_idx, data_samples in tqdm(enumerate(dataloader),
        ...
        for image, data_sample in zip(images, data_samples):
            ...
            category = data_sample.pop('category')
            # os.makedirs(f"{args.output}/{sample_id:05d}", exist_ok=True)
            os.makedirs(args.output,exist_ok=True)
            index = data_sample.pop('index')
            ...
            outpath = os.path.join(args.output, category, f"{index}.png")
